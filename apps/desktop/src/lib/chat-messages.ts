...
import type { MessageReaction, SessionMessage, SessionTurnOutcome, UsageStats } from '@/types/hermes'
...
export function toChatMessages(messages: SessionMessage[], turnOutcomes: SessionTurnOutcome[] = []): ChatMessage[] {
  ...
  withTurnOutcomeRows(messages, turnOutcomes).forEach((message, index) => {
    ...
    result.push({
      // A synthetic turn-outcome row is keyed by its outcome id so the same
      // outcome can't be re-added under a new index on the next rehydrate.
      id: outcomeId ? `turn-outcome:${outcomeId}` : `${message.timestamp || Date.now()}-${index}-${displayRole}`,
      role: displayRole,
      parts,
