...
describe('usageFromTokenUsagePayload', () => {
  ...
  it('derives context percent when the backend omits context_pct', () => {
    ...
    ).toMatchObject({ context_max: 200_000, context_percent: 25, context_used: 50_000 })
  })
  ...
  it('keeps live context usage from moving backwards on an older same-turn snapshot', () => {
    const current = {
      calls: 2,
      context_max: 100_000,
      context_percent: 67,
  ...
  it('allows context usage to drop when compression advances', () => {
    ...
    ).toMatchObject({ compressions: 1, context_percent: 22, context_used: 22_000 })
  })

  it('allows an explicit session switch to lower context usage', () => {
    const current = {
      calls: 2,
      context_max: 100_000,
      context_percent: 67,
    ...
    ).toMatchObject({ context_percent: 12, context_used: 12_000, total: 10_100 })
  })
})
