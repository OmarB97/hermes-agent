      ...
      try {
        ...
        if (prefetchedResult) {
          ...
          localSnapshot = reconcileAuthoritativeMessages(
            ...
            storedSessionId,
            undefined,
            prefetchedResult.turn_outcomes
          )
        ...
        // Seed the turn-outcome ledger from the resume payload so a turn that
        // ended while the app was away still renders its outcome (#270).
  ...
  const archiveSession = useCallback(
    async (storedSessionId: string) => {
      ...
      try {
        ...
        // not appear to do nothing until the next full refresh. (beginSessionMutation
        // above covers the projects.tree prune; this covers the flat list.)
        setSessions(prev => prev.filter(session => !sessionMatchesStoredId(session, storedSessionId)))
