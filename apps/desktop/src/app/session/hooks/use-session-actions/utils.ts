...
const IGNORED_FIELDS = ['timestamp', 'attachmentRefs', 'parts', 'rowId', 'senderDevice'] as const
...
/** The persisted counters a stored session row already carries, shaped as the
 *  usage snapshot the context meter reads. Used to seed a resume so a warm
 *  agent reporting partial runtime usage can't blank the context bar (#316). */
...
interface ApplyRuntimeInfoOptions {
  ...
  foreground?: boolean
  /**
   * Persisted counters to seed the usage merge from — normally
   * {@link storedSessionUsagePreview} of the row being resumed.
   */
  usageFallback?: UsageStats
}

/** Mirror a session's runtime state into the atoms the MAIN pane renders from.
 *  Foreground sessions only — see ApplyRuntimeInfoOptions.
 *
 *  Model / provider / reasoning effort / fast mode go to the ACTIVE-SESSION
 *  mirror, never to the composer's persisted pick: session.create echoes a
 *  spawn's `-m` / `--effort` overrides straight back as runtime info
 *  (tui_gateway/server.py), so treating them as a selection re-aimed every chat
 *  the user started afterwards (#298, #318, #322). */
function publishRuntimeToComposer(state: SessionRuntimeStatePatch): void {
  ...
  if (state.fallbackPolicy !== undefined) {
  ...
  if (state.fast !== undefined) {
    setActiveSessionFastMode(state.fast)
  }
...
export function applyRuntimeInfo(
  ...
  { foreground = true, usageFallback = emptyUsageStats() }: ApplyRuntimeInfoOptions = {}
  ...
  if (info.usage) {
    ...
    // disappear during an otherwise-successful session switch (#316).
    sessionState.usage = mergeUsageSnapshot(usageFallback, info.usage, { allowContextDecrease: true })
  }
  ...
  if (foreground) {
    ...
    if (sessionState.usage) {
      setCurrentUsage(sessionState.usage)
    }
  }
