...
export function usePromptActions({
  ...
  const reloadFromMessage = useCallback(
    async (parentId: string | null) => {
      ...
      clearNotifications()
      // The ref-read id, never the closure-captured prop: a regenerate aimed at
      // a stale session paints (and then truncates) the wrong transcript.
      const regenerateState = updateSessionState(sessionId, state => applyReloadOptimistic(state, plan))
