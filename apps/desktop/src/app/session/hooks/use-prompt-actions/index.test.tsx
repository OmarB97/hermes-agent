...
describe('usePromptActions slash.exec dispatch payloads', () => {
  ...
  // /compress compresses through the dedicated session.compress RPC (see the
  // /compress describe above), and the backend still refuses to mutate a
  // transcript a live turn owns. The desktop keeps the command atomic: the
  // interrupt happens on this side so the user never has to hand-run
  // /interrupt and retry.
  it('interrupts an active turn before dispatching /compress', async () => {
    ...
    const requestGateway = vi.fn(async (method: string) => {
      ...
      if (method === 'session.compress') {
        return { removed: 42, summary: { headline: 'Compressed: 42 messages to 8' } } as never
      }
    ...
    expect(methods).toEqual(['session.interrupt', 'session.compress'])
    ...
    // The slash-worker route is the older gateway's fallback, never the path a
    // busy-edge interrupt takes.
    expect(requestGateway).not.toHaveBeenCalledWith('slash.exec', expect.anything())
    expect(requestGateway).not.toHaveBeenCalledWith('command.dispatch', expect.anything())
  })
  ...
  it('recovers when the gateway still reports busy at the idle edge', async () => {
    ...
    // The desktop thinks the session is idle, the gateway disagrees: interrupt
    // once, then ride the settle edge out on the retry instead of surfacing a
    // "session busy" the user would have to act on.
    expect(methods).toEqual(['session.compress', 'session.interrupt', 'session.compress'])
    expect(compressAttempts).toBe(2)
  })
})
...
describe('usePromptActions submit / queue drain semantics', () => {
  ...
  it('a null-runtime queue drain resumes its ORIGIN session instead of hijacking the foreground one', async () => {
    ...
    expect(requestGateway).toHaveBeenCalledWith(
      'prompt.submit',
      { queued: true, session_id: 'rt-session-june', text: 'what about the season posters?' },
      1_800_000
    )
