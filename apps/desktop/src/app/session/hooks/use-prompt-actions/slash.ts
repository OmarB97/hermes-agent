    ...
    async (rawCommand: string, options?: { sessionId?: string; recordInput?: boolean }) => {
      ...
      const actionHandlers: Record<DesktopActionId, (ctx: SlashActionCtx) => Promise<void>> = {
        ...
        compress: async ctx => {
          ...
          // Compression mutates the persisted transcript, so the backend
          // correctly refuses it while a turn owns that history (4009 "session
          // busy"). Make the desktop command atomic from the user's point of
          // view: interrupt the live turn, ride out the gateway's short
          // running->idle settle edge, then compress once. This keeps the
          // backend's data-loss guard intact without making users hand-run
          // /interrupt and retry /compress themselves.
          ...
          try {
            ...
            try {
              ...
              // Only the busy guard is recoverable here. Anything else —
              // above all the method-not-found of a gateway that predates
              // session.compress — must reach the outer catch untouched, or
              // the slash-worker fallback below never runs.
            ...
            // immediate "method not found" error. A live turn was already
            // interrupted above, so the legacy route stays atomic too.
            if (isMissingRpcMethod(err)) {
              await runExec(ctx)
