  ...
  return useCallback(
    (event: RpcEvent) => {
      ...
      if (event.type === 'gateway.ready') {
        // The backend names this device in its hello; sender attribution on
        // outgoing prompts (submit.ts reads $localDeviceName) needs it, and the
        // first hello wins so a reconnect can't rename an in-flight session.
        ...
        // No global $currentUsage write here on purpose: session.info usage now
        // merges into the session's OWN state above (mergeUsageSnapshot), and
        // the focused session's state is what syncRuntimeMetadataToView mirrors
        // into $currentUsage. Writing the raw payload here as well let a
        // background heartbeat overwrite the viewed session's context meter
        // mid-turn (#316).
        ...
        // Track the turn this session is running so a timed-out delegated turn
        // can be matched back to its own loop (#319).
