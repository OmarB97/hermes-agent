...
import { shieldSessionFromMerge } from '@/store/session-states'
...
export async function archiveStoredSessions(
  sessionIds: string[],
  options: BulkOptions = {}
  ...
  const results = await Promise.allSettled(targets.map(t => setSessionArchived(t.session.id, true, t.session.profile)))
...
export async function restoreArchivedSessions(
  sessionIds: string[],
  options: BulkOptions = {}
): Promise<BulkSessionResult> {
  const targets = [...new Set(sessionIds)]
  ...
  const results = await Promise.allSettled(targets.map(t => setSessionArchived(t.session.id, false, t.session.profile)))
