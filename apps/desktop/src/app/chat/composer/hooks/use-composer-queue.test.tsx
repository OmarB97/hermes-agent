...
describe('useComposerQueue park integration', () => {
  ...
  it('sendQueuedNow while busy unparks so the settle drain flows (no stale latch)', async () => {
    ...
    // Turn settles → the promoted entry drains, leading the queue. The drain
    // preflights the entry's attachments before submitting, so the send lands a
    // microtask after the settle rather than inside the rerender; and because
    // this harness pins `busy` false (a real send flips it back to true) the
    // rest of the queue follows behind it.
    hook.rerender({ busy: false })

    await waitFor(() => expect(getQueuedPrompts(SESSION_KEY)).toHaveLength(0))

    // Promoted entry first, every entry sent exactly once, park never relatched.
    expect(onSubmit.mock.calls.map(call => call[0])).toEqual(['send me now', 'first'])
    expect(isQueueParked(SESSION_KEY)).toBe(false)
  })
})
