          ...
          {groups.map(group => (
            <DropdownMenuGroup className="py-0.5" key={group.provider.slug}>
              ...
              {group.families.map(family => {
                ...
                // Grayed text reflects each row's effective state: Fast only when
                // that row's preset (or live session) actually has it on, never
                // from a fast-capable sibling alone, and effort only for models
