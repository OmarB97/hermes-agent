...
// Exact listable session count across the active scope. Upstream dropped this
// in favour of $sessionProfilesTruncated (below); the fork keeps it because the
// sidebar renders a real "N of M" footer and bulk archive/delete adjusts the
// count in place instead of refetching (see session-bulk-actions).
export const $sessionsTotal = atom<number>(0)
// Archived sessions are their own list, rendered as the sidebar's bottom
// section, so archiving a chat moves it out of recents without a refetch.
...
// Exact listable conversation count per profile (children excluded), keyed by
// profile name. Upstream replaced this with the truncated flag above; the fork
// keeps both because its sidebar renders real per-profile counts and its bulk
// actions decrement them in place (adjustSessionProfileTotal) rather than
// refetching a page after every archive/delete.
...
export const adjustSessionProfileTotal = (profileKey: string, delta: number) => {
  ...
  if (profileKey in current) {
    $sessionProfileTotals.set({
      ...current,
      [profileKey]: Math.max(0, (current[profileKey] ?? 0) + delta)
    })
  }
}
...
export const adjustMessagingPlatformTotal = (sourceId: string, delta: number) => {
  ...
  if (sourceId in current) {
    $messagingPlatformTotals.set({
      ...current,
      [sourceId]: Math.max(0, (current[sourceId] ?? 0) + delta)
    })
  }
}
