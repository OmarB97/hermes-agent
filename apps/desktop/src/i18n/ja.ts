...
export const ja = defineLocale({
  ...
  sidebar: {
    ...
    attention: {
      ...
      kindSecret: 'シークレット',
      kindSudo: 'Sudo',
      target: label => `${label} 宛て`,
      targetAnyMember: '任意のメンバー',
      targetOriginator: '依頼者',
      targetOwnerAdmin: 'オーナー/管理者',
      targetPrincipal: '指定された人',
      targetRole: '指定されたロール',
      targetUnknown: '応答可能な参加者',
      from: actor => `${actor} から`,
      fromVia: (actor, via) => `${actor} から ${via} 経由`,
      via: via => `${via} 経由`
    },
    cronJobs: 'Cronジョブ',
