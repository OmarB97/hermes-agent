...
export const en: Translations = {
  ...
  boot: {
    ...
    errors: {
      ...
      gatewayUnreachable: "Can't reach the gateway",
  ...
  settings: {
    ...
    toolsets: {
      ...
      postSetupCompleteMessage: (step: string) => `The setup step "${step}" completed successfully.`,
      postSetupErrorTitle: 'Setup failed',
      postSetupErrorMessage: (step: string) => `The setup step "${step}" failed with an error.`,
      postSetupFailed: (step: string) => `Setup step "${step}" failed. Run it from the CLI with hermes tools.`
    }
  },
  ...
  sidebar: {
    ...
    sessions: 'Sessions',
    cronJobs: 'Cron jobs',
    liveElsewhere: 'Live on other devices',
    ...
    bulk: {
      ...
      restore: 'Restore',
      delete: 'Delete',
      promptCount: count => (count === 1 ? 'Prompt 1' : `Prompt ${count}`),
      steerCount: count => (count === 1 ? 'Steer 1' : `Steer ${count}`),
      haltCount: count => (count === 1 ? 'Stop 1' : `Stop ${count}`),
      archiveCount: count => (count === 1 ? 'Archive 1' : `Archive ${count}`),
      restoreCount: count => (count === 1 ? 'Restore 1' : `Restore ${count}`),
      deleteCount: count => (count === 1 ? 'Delete 1' : `Delete ${count}`),
      promptDialogTitle: count => (count === 1 ? 'Prompt 1 session' : `Prompt ${count} sessions`),
      promptDialogDesc: 'Send the same message to every selected session.',
      promptPlaceholder: 'Message to send…',
      promptSubmit: 'Send prompt',
      steerDialogTitle: count => (count === 1 ? 'Steer 1 session' : `Steer ${count} sessions`),
      steerDialogDesc: 'Nudge the currently running turn in every selected session.',
      steerPlaceholder: 'Steering note…',
      steerSubmit: 'Steer',
      ...
      promptedToast: count => (count === 1 ? 'Prompt sent to 1 session' : `Prompt sent to ${count} sessions`),
      steeredToast: count => (count === 1 ? 'Steered 1 session' : `Steered ${count} sessions`),
      haltedToast: count => (count === 1 ? 'Stopped 1 session' : `Stopped ${count} sessions`),
      ...
      promptFailed: count => (count === 1 ? 'Could not prompt 1 session' : `Could not prompt ${count} sessions`),
      steerFailed: count => (count === 1 ? 'Could not steer 1 session' : `Could not steer ${count} sessions`),
      haltFailed: count => (count === 1 ? 'Could not stop 1 session' : `Could not stop ${count} sessions`),
  ...
  updates: {
    ...
    availableBodyBackend: 'A new Hermes backend version is ready to deploy.',
    availableBodyNoChangelog: 'Backend has updated — details not available.',
