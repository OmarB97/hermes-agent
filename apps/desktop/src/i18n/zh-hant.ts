...
export const zhHant = defineLocale({
  ...
  sidebar: {
    ...
    attention: {
      ...
      kindSecret: '密鑰',
      kindSudo: 'Sudo',
      target: label => `給 ${label}`,
      targetAnyMember: '任何成員',
      targetOriginator: '發起者',
      targetOwnerAdmin: '擁有者/管理員',
      targetPrincipal: '指定人員',
      targetRole: '指定角色',
      targetUnknown: '可回應的參與者',
      from: actor => `來自 ${actor}`,
      fromVia: (actor, via) => `來自 ${actor} 透過 ${via}`,
      via: via => `透過 ${via}`
    },
    cronJobs: '排程任務',
