...
class ContextProfile:
    ...
    # Fork feature: categories fully pruned from the prompt's skill index in the
    # coding posture (deny-list). Discovery-only — pruned skills stay loadable
    # via skills_list/skill_view. Kept alongside upstream's softer
    # compact_skill_categories (names-only demotion under ``focus``); see
    # build_skills_system_prompt for how both are applied.
...
class RuntimeMode:
    ...
    def hidden_skill_categories(self) -> frozenset[str]:
        """Skill categories the fork prunes from the prompt's skill index.

        Empty outside the coding posture (GENERAL_PROFILE carries none).
        Discovery-only: pruned skills stay loadable via ``skills_list`` /
        ``skill_view``. Applies in any coding mode, unlike the ``focus``-gated
        ``compact_skill_categories`` above; ``build_skills_system_prompt`` prunes
        these first, then demotes whatever survives.
        """
...
def coding_hidden_skill_categories(
    *,
    ...
    Empty outside the coding posture. Discovery-only: pruned skills remain
    loadable via ``skills_list`` / ``skill_view``. Fork feature kept alongside
    upstream's ``coding_compact_skill_categories``; ``build_skills_system_prompt``
    prunes these first, then demotes whatever survives.
    """
    return resolve_runtime_mode(
        platform=platform, cwd=cwd, config=config
