...
def build_skills_system_prompt(
    ...
    # Posture-driven category handling (e.g. non-coding skills while pairing on
    # code). Two complementary, opt-in mechanisms — ``hidden_categories`` prunes
    # whole categories from the index (fork's coding posture), while
    # ``compact_categories`` demotes them to a names-only line (upstream's focus
    # mode). Both match on the top-level category segment so nested categories
    # ("social-media/twitter") are handled with their parent.
    hidden_note = ""

    # Pruning first: drop whole categories from the rendered index. Discovery-
    # only — the snapshot keeps everything and skills_list/skill_view still
    # reach every skill, so a footer note points at the full catalog.
    ...
    # Demotion next, on whatever survived pruning: demoted categories stay in
    # the index as a single names-only line — descriptions are dropped to cut
    # noise, but every skill name remains visible so memory-anchored recall
    # ("load <name>") keeps working. NEVER remove entries entirely: agent-
    # created skills are the model's project memory, and models don't reach for
    # skills_list to rediscover what the index stops showing them.
    demoted = frozenset(
