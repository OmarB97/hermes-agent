...
def _record_accepted_provider_response(
    ...
    *,
    ...
    # Stash this response's canonical usage so the post-turn on_turn_complete()
    # observation hook can forward it — the same dict shape record_canonical_usage
    # feeds to update_from_response. A turn may make several API calls; the
    # engine's per-turn signal of interest is the cost/size of the latest
    # assembled request, so keep the most recent call's usage.
