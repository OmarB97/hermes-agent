    ...
    while (api_call_count < agent.max_iterations and agent.iteration_budget.remaining > 0) or agent._budget_grace_call:
        ...
        while retry_count < max_retries:
            ...
            try:
                ...
                # Snapshot the pre-flight prompt-size estimate to state.db
                # so downstream dashboards can show in-flight context burn
                # during the gap between request-send and response-commit.
                # Cleared by update_token_counts on success; cleared in the
                # api_error except branch on failure. Best-effort.
                if agent._session_db and agent.session_id:
                    try:
                        agent._session_db.set_pending_prompt_tokens(
                            agent.session_id, approx_tokens,
                        )
                    except Exception:
                        pass
                    ...
                    if agent._session_db and agent.session_id:
                        try:
                            ...
                            agent._session_db.update_token_counts(
                                ...
                                model=agent.model,
                                api_call_count=1,
                                # Per-call snapshots for the dashboard
                                # context chip — current-context size
                                # (prompt) + most recent completion.
                ...
                # Clear pending_prompt_tokens — this call's pre-flight
                # estimate is dead, downstream dashboards shouldn't keep
                # showing in-flight context for a request that errored.
                # Next retry's pre-flight write resets cleanly.
                if agent._session_db and agent.session_id:
                    try:
                        agent._session_db.set_pending_prompt_tokens(
                            agent.session_id, 0,
