...
# The managed production W2 lane can spend longer than the generic DFlash
# deadline in gate admission plus cold prefill even when the request is
# healthy. Keep this route/model exception bounded and narrower than the CLI's
# 420s outer acceptance window; explicit provider/model config and legacy
# overrides still win below.
_MANAGED_W2_FIRST_CHUNK_TIMEOUT_DEFAULT_S = 360.0
...
def interruptible_streaming_api_call(agent, api_kwargs: dict, *, on_first_delta=None):
    ...
    def _call_chat_completions(stream_attempt_id: int):
        ...
        def _open_stream(next_api_kwargs: dict[str, Any]):
            ...
            # Reset the stale/first-chunk timers so each attempt is measured
            # from ITS own start, not a previous attempt's last chunk — the
            # local DFlash first-chunk watchdog reads both.
            last_chunk_time["t"] = time.time()
            first_chunk_seen["yes"] = False
        ...
        def _accept_stream_chunk(_chunk: Any) -> bool:
            ...
            # event.
            last_chunk_time["t"] = time.time()
            # First real chunk closes the pre-first-chunk phase: the local
            # DFlash watchdog stops applying its (much larger) TTFB budget and
            # the stale detector takes over from here.
    ...
    while t.is_alive():
        ...
        if _hb_now - _last_heartbeat >= _HEARTBEAT_INTERVAL:
            ...
            if (
                _waiting_secs >= _HEARTBEAT_INTERVAL
                and not (
                    _dflash_first_chunk_timeout is not None
                    and not first_chunk_seen["yes"]
                )
            ):
                _recovery = (
                    f"; auto-reconnect at {int(_stream_stale_timeout)}s"
                    if math.isfinite(_stream_stale_timeout)
                    else ""
                )
                agent._emit_wait_notice(
                    f"⏳ waiting on {api_kwargs.get('model', 'the provider')} — "
        ...
        if _dflash_first_chunk_timeout is not None and not first_chunk_seen["yes"]:
            ...
            if _first_elapsed > _dflash_first_chunk_timeout:
                ...
                _request_cancelled["value"] = True
                try:
                    _cancel_current_stream_attempt("dflash_first_chunk_kill")
