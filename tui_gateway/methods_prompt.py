...
def _(rid, params: dict) -> dict:
    ...
    with session["history_lock"]:
        ...
        if truncate_user_ordinal is not None:
            ...
            # raised "FOREIGN KEY constraint failed" and the profile db kept the
            # FULL pre-edit history — so resuming brought the edited-away turns
            # straight back. Same class as the /undo rewind in #323.
            with _session_db(session) as db:
                if db is not None:
                    try:
                    ...
                    # The UI-only outcome rows for the turns just undone must
                    # stop rendering too, or a rewind leaves the edited-away
                    # turns' terminal notes in the transcript. Best-effort on
                    # its own: the transcript rewrite above already committed,
                    # so a failure here is stale display rows, not zombie
                    # history — not worth failing the turn the user just sent.
                    try:
                        ...
                        logger.warning(
                            "prompt.submit: failed to deactivate turn outcomes "
                            "for session %s (ordinal=%d)",
                            sid,
                            ordinal,
                            exc_info=True,
                        )
        ...
        # One critical section claims the session AND mints the turn identity
        # (running flag + inflight_turn + cleared cancel latch), so two
        # post-turn producers can't both believe they own the next dispatch.
    ...
    def run_after_agent_ready() -> None:
        ...
        if err:
            ...
            # Durable terminal outcome for the same failure. _emit_terminal_turn_error
            # is the live frame (lost on a disconnect); this is the persisted row
            # session.resume rehydrates from. _fail_inflight_turn retains the turn,
            # so the id this freezes against is still there.
        ...
        if cancelled:
            # Persist + emit the terminal outcome BEFORE the in-flight identity is
            # cleared — freezing an outcome needs the turn id that clear removes.
            _finalize_turn_outcome(
                sid,
                session,
                status="cancelled",
            ...
            with session["history_lock"]:
                ...
                _emit("error", sid, {"message": cancel_message})
            return
        # Dispatch the turn this handler already claimed. _dispatch_claimed_prompt
        # owns the failure path: a dispatch that raises finalizes the turn and
        # releases the running flag instead of stranding the session busy.
        with session["history_lock"]:
        ...
        try:
            ...
            result = AIAgent(
                **_background_agent_kwargs(session["agent"], task_id),
                # Relay the background agent's progress onto the PARENT session's
                # status line — otherwise a background task is silent until it
                # completes.
