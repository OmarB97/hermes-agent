...
def _ensure_session_db_row(session: dict) -> None:
    ...
    * A terminal session (``hermes`` / ``hermes --tui`` / CLI) is started from a
    ...
    # Attribute the row to its profile from the start. The agent backfills this
    # on its first turn (run_agent.py), so today an unattributed row is only
    # briefly wrong — but a session whose turn never runs (a spawn that dies
    # before its first prompt) keeps a NULL profile_name forever.
    if profile_home:
        # A profile's directory name IS its identity — the same rule
        # session.branch uses when it stamps a branch row. Asking
        # get_active_profile_name() under the bound home instead loses that
        # identity for any home outside ``<root>/profiles/<name>``: it answers
        # the literal "custom", which then reaches the unified list as a
        # profile nobody owns.
        row_profile_name = Path(profile_home).name or None
    else:
        # Launch profile: no home to name it, so ask the ambient one.
...
def _enqueue_prompt(
    ...
    *,
    ...
    if (
        ...
        prev = existing["text"]
        if front:
            # Front-insert (a /steer that landed after the final tool batch, a
            # re-queued turn): this text has to run BEFORE what is already
            # waiting, and the reply belongs to the transport that owns the
            # existing slot.
    ...
    if existing:
        if front:
            # ``queued_prompt`` is the head of the queue and ``queued_prompts``
            # the tail, so a real front-insert displaces the head rather than
            # jumping the tail — otherwise a restored/steered turn would still
            # run second.
...
def _dispatch_claimed_prompt(
    ...
    *,
    ...
    if display_kind:
        submit_kwargs["display_kind"] = display_kind
        submit_kwargs["display_metadata"] = display_metadata
    if image_paths:
        submit_kwargs["image_paths"] = image_paths
    if queued_prompt_generation is not None:
        submit_kwargs["queued_prompt_generation"] = queued_prompt_generation
    try:
        if _session_uses_compute_host(session):
            host_kwargs = {
                k: v
                for k, v in submit_kwargs.items()
                if k in {"image_paths", "queued_prompt_generation"}
            }
            response = _submit_prompt_to_compute_host(
                rid, sid, session, text, **host_kwargs
            )
            if response.get("error"):
                message = str(
                    ((response.get("error") or {}).get("message"))
                    or f"{failure_context} failed"
                )
...
def _drain_queued_prompt(rid, sid: str, session: dict) -> bool:
    ...
    with session["history_lock"]:
        if int(session.get("_queued_prompt_generation", 0)) != queue_generation:
            # The queue was cleared (or rotated) while we were claiming it —
            # release the turn we just opened instead of running a prompt the
...
def _dispatch_notification_batch(
    ...
    for index, (event, text) in enumerate(notifications):
        claim = claim_event_delivery(event, "tui-post-turn")
        if claim is None:
            continue
        with session["history_lock"]:
            if session.get("running"):
...
def _lazy_resume_info(
    ...
    *,
    ...
    with _profile_home_bound(profile_home):
        ...
        # Named profile wins when the RPC supplied one; otherwise the bound
        # home names it (and, unbound, the launch profile) — same answer both
        # sides of the merge, without needing the caller to pass both.
...
def _live_session_payload(
    ...
    *,
    ...
    payload = {
        ...
        "message_count": len(history),
        "messages": (
            [] if omit_messages else _history_to_messages(history, turn_outcomes)
        ),
...
def _notification_poller_loop(
    ...
    while not stop_event.is_set() and not session.get("_finalized"):
        ...
        if _dispatch_claimed_prompt(
            ...
            **_async_delegation_display_kwargs(evt),
        ):
...
def _async_delegation_display_kwargs(evt: dict) -> dict:
    """Display typing for a notification-poller dispatch, or {} for other events.

    An async-delegation completion is synthetic scaffolding, not a user turn, so
    the row it persists has to carry ``display_kind`` /``display_metadata`` for
    every surface to render it as a delegation card instead of a chat bubble.
    Returned as kwargs because the dispatch helper is shared with plain
    notifications, which take neither.
    """
    if evt.get("type") != "async_delegation":
        return {}
    return {
    ...
    *,
    ...
    with session["history_lock"]:
        if (
            ...
            # The queue was cleared after this turn was claimed. The caller may
            # already have opened an in-flight turn for it (see
            # _begin_prompt_dispatch_locked), so release the claim instead of
            # leaving a phantom streaming turn the next submit would append to.
            _clear_inflight_turn(session)
            session["running"] = False
            return
        if image_paths is None:
    ...
    def run():
        ...
        terminal_turn = None
        # Pre-bound so the finally's history.clear() cannot raise
        # UnboundLocalError when the turn fails before the snapshot below —
        # that would abort the finally and leave the session stuck running.
        history: list = []
        ...
        try:
            ...
            if reaction_notes := _pending_reaction_notes(session):  # noqa: F821 — bound onto this namespace by methods_prompt install()
                if isinstance(run_message, str):
            ...
            # The outcome must capture the runtime that executed the turn,
            # before a one-turn model override is restored.
            _finalize_turn_outcome(
                sid,
                session,
                result=result,
