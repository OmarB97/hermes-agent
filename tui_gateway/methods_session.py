...
def _(rid, params: dict) -> dict:
    ...
    # The default model reported below must answer for the profile this chat was
    # created *under*. This handler runs on the RPC thread with nothing bound, so
    # unbound it reports the launch profile's model and the client sees it flip
    # once the deferred build (which does bind the home) emits its session.info.
    # (The profile name field is answered by _response_profile_name, which
    # resolves the requested profile directly and needs no binding.)
...
def _(rid, params: dict) -> dict:
    ...
    if is_truthy_value(params.get("lazy", False)):
        ...
        # UI-only turn outcomes merge into the SAME display projection, never
        # into the model-fed history. Child-only (no ancestors), matching the
        # include_ancestors=False reads above.
        turn_outcomes = _turn_outcomes_for_display(db, target)
        messages = (
            [] if omit_messages else _history_to_messages(display_history, turn_outcomes)
        )
        return _ok(
            rid,
            {
    ...
    if not is_truthy_value(params.get("eager_build", False)):
        ...
        try:
            ...
            # UI-only turn outcomes for the display projection. Lineage-wide,
            # because the display copy above is the lineage read.
    ...
    try:
        ...
        if omit_messages:
            ...
            raw_history, display_history = db.get_resume_conversations(target)
        # UI-only turn outcomes for the display projection (lineage-wide, like
        # the display read above). Never enters the model-fed history.
        turn_outcomes = _turn_outcomes_for_display(db, target, include_ancestors=True)
        ...
        with _session_db(session) as db:
            if db is not None:
                try:
                    ...
                    # UI-only outcomes for the same lineage read. Left empty when
                    # the history read failed, so outcomes never merge into the
                    # live-session fallback copy with mismatched ordinals.
                    turn_outcomes = _turn_outcomes_for_display(
...
def _(rid, params: dict) -> dict:
    ...
    with _session_db(session) as db:
        ...
        try:
            # Bind the parent's home for the row write: _resolve_model() below
            # reads config.yaml, and unbound it stamps the LAUNCH profile's
            # default model into the branch's row — the same mis-binding #325
            # fixed for _ensure_session_db_row, made permanent here by the
            # row's COALESCE upsert. (_session_db above opens the profile's
            # handle but does not bind its HERMES_HOME.)
            with _profile_home_bound(session.get("profile_home")):
                if branch_name:
                    title = branch_name
                else:
...
def _(rid, params: dict) -> dict:
    ...
    with session["history_lock"]:
        session["_turn_cancel_requested"] = True
        # The reason the turn's terminal outcome reports. _freeze_turn_outcome
        # reads it back; without it a user Stop lands as a bare "cancelled".
