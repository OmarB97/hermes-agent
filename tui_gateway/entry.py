...
def main():
    ...
    # the resilient stdin loop below.  Distinct from the change watcher above:
    # that one polls on-disk config signatures (a skin the agent switched or
    # edited), this one polls the OS light/dark setting so `display.skin: auto`
    # re-resolves when the desktop theme flips (#257).
    try:
