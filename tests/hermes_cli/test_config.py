...
class TestCustomProviderCompatibility:
    ...
    def test_v11_remigration_merges_identical_existing_provider(self, tmp_path):
            ...
            yaml.safe_dump(
                {
                    ...
                    "custom_providers": [
                        ...
                        # Env-backed credential ref on the incoming side only
                        # (operator's actual ai-router shape) → merge and
                        # carry the key_env ref instead of dropping it.
                        {
                            "name": "ai-router",
                            "base_url": "http://10.10.20.199:9081/v1",
                            "key_env": "HERMES_LLM_API_KEY",
                        },
        ...
        assert raw["providers"]["ai-router"]["key_env"] == "HERMES_LLM_API_KEY"
        assert "custom_providers" not in raw
