...
class TestWebServerEndpoints:
    ...
    def test_get_session_messages_follows_compression_tip(self):
        ...
        try:
            ...
            # The root deliberately KEEPS its pre-compression transcript, which
            # is what publish_compression_child does in production: the parent
            # is closed, never emptied. (Writing to a compression-closed parent
            # — replace_messages/append_message — now raises
            # CompressionSessionClosedError by design.) Leaving the row intact
            # also sharpens the assertion below: an endpoint that failed to
            # follow the tip would return "before compression", not an empty
            # list that could pass for any number of reasons.
