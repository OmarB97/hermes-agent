        ...
        try:
            ...
            if callable(_post_cb):
                try:
                    ...
                    pass
            # Some adapters keep platform-level typing tasks (e.g. Discord).  If
            # callback work or a late refresh recreated one, make one final
            # bounded stop before releasing the session guard.  _stop_typing_refresh
            # calls stop_typing() internally, so this also covers the fork's
            # persistent-typing cancellation without a separate manual call.
            await self._stop_typing_refresh(
                event.source.chat_id,
                None,
                stop_attempts=1,
