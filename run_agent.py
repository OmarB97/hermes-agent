...
class AIAgent:
        ...
        if not self._session_db_created:
            ...
            try:
                try:
                    ...
                    if profile_for_session == "default":
                ...
                try:
                    ...
                    row_pre_existed = True
                # Carry the live YOLO bypass into the creation-time model_config
                # so a session whose /yolo was toggled BEFORE the row existed
                # (the row is created lazily on the first turn) still persists
                # the flag for `hermes --resume`. set_session_yolo() no-ops on a
                # missing row, so this is the only chance to record a
                # pre-first-turn toggle.
                init_model_config = self._session_init_model_config
                try:
                    from tools.approval import is_session_yolo_enabled
                    if is_session_yolo_enabled(self.session_id):
                        init_model_config = dict(init_model_config or {})
                        init_model_config["yolo_mode"] = True
                except Exception:
                    pass
                self._session_db.create_session(
                    ...
                    profile_name=profile_for_session,
                )
    ...
    def clear_interrupt(self, *, preserve_redirect: bool = False) -> bool:
        ...
        # cancellations noticed outside ``interrupt()`` itself. A fallback
        # notice belongs only to the turn that activated it, so never let the
        # next primary success announce a stale provider switch.
        self._pending_fallback_notice = None
        self._interrupt_thread_signal_pending = False
        if self._execution_thread_id is not None:
    ...
    def run_conversation(
        ...
        try:
            ...
            # Fork: every terminal result carries the durable session
            # telemetry counters (#274). Mutates and returns ``result``.
