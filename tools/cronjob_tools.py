...
def _resolve_model_override(model_obj: Optional[Dict[str, Any]]) -> tuple:
    ...
    Callers are the user-owned pin surfaces — ``hermes cron create/edit --model``
    (``hermes_cli.cron._cron_inference_args``) and the dashboard. The agent's
    ``cronjob`` tool deliberately does not route model/provider at all (see the
    registry handler at the bottom of this module), so an unattended job can
    never be re-pointed at a different model by the model itself.
