    ...
    def _init_schema(self):
        ...
        if row is None:
            ...
            if current_version < 18:
                # v18: tag delegate subagent rows so pickers stay clean after
                # parent deletes that used to orphan them (parent_session_id → NULL).
                # Upstream v0.17.0 shipped this as v16; renumbered to v18 here so it
                # lands after the fork's own v16/v17 and existing fork DBs (at v17)
                # still apply it.
                try:
                    cursor.execute(
        ...
        def _do(conn):
            cursor = conn.execute(
                """INSERT INTO messages (session_id, role, content, tool_call_id,
                   ...
                   codex_message_items, platform_message_id, sender_device, observed, active)
                   VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""",
                (
                    session_id,
                    role,
