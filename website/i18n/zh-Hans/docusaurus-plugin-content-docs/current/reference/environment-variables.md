...
| `HERMES_LOCAL_STREAM_STALE_TIMEOUT` | 本地提供商（Ollama、oMLX、llama-cpp）的过期流上限（秒，默认：`900`）。仅在检测到本地端点且没有更具体的设置时生效——`providers.<id>.stale_timeout_seconds` 和 `HERMES_STREAM_STALE_TIMEOUT` 都优先生效。这个有限上限取代了此前的"无限禁用"，使卡死的本地服务最终会触发检测器，而不是永远挂起；上限会按该请求上下文的预填充成本放宽（只增不减），因此大提示词不会在预填充途中被杀掉。设为 `0` 可完全禁用。也可通过 `config.yaml` 中的 `agent.local_stream_stale_timeout` 配置（推荐使用该配置项，本环境变量仅作为应急开关）。 |
