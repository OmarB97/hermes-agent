...
| `HERMES_API_CALL_STALE_TIMEOUT` | 非流式过期调用超时（秒，默认：`300`）。未设置时对本地提供商自动禁用——但本地 DeepSeek Flash 系列以及思考阶段较长的模型（DeepSeek R1、QwQ、Qwen3、Nemotron 3、o 系列）仍保留有限预算。当该检测在本地端点上生效时，若后端提供进度信号，终止请求前会先询问其是否仍在生成；如果是，则延长等待，上限由 `HERMES_DFLASH_FIRST_CHUNK_CEILING`（默认 `1800`）限制。没有该信号的端点（普通 Ollama 或 llama.cpp）仍仅由计时器决定。也可通过 `config.yaml` 中的 `providers.<id>.stale_timeout_seconds` 或 `providers.<id>.models.<model>.stale_timeout_seconds` 配置。 |
| `HERMES_STREAM_READ_TIMEOUT` | 流式 socket 读取超时（秒，默认：`120`）。对本地提供商自动增大到 `HERMES_API_TIMEOUT`。如果本地 LLM 在长代码生成期间超时，请增大此值。 |
| `HERMES_STREAM_STALE_TIMEOUT` | 过期流检测超时（秒，默认：`180`）。在此窗口内无块到达时触发连接终止。本地提供商改用更宽松的 `HERMES_LOCAL_STREAM_STALE_TIMEOUT` 上限；若显式设置本变量，则你设定的值对所有提供商生效。 |
| `HERMES_LOCAL_STREAM_STALE_TIMEOUT` | 本地提供商（Ollama、oMLX、llama-cpp）的过期流上限（秒，默认：`900`）。仅在检测到本地端点且没有更具体的设置时生效——`providers.<id>.stale_timeout_seconds` 和 `HERMES_STREAM_STALE_TIMEOUT` 都优先生效。这个有限上限取代了此前的"无限禁用"，使卡死的本地服务最终会触发检测器，而不是永远挂起；上限会按该请求上下文的预填充成本放宽（只增不减），因此大提示词不会在预填充途中被杀掉。上限到期时，若本地后端提供进度信号，会先询问它是否仍在生成；若仍在生成则继续等待，并由 `HERMES_DFLASH_FIRST_CHUNK_CEILING`（默认 `1800`）限定单次静默最长时长。没有该信号的端点（如普通的 Ollama 或 llama.cpp）仍只按计时器判断。设为 `0` 可完全禁用。也可通过 `config.yaml` 中的 `agent.local_stream_stale_timeout` 配置（推荐使用该配置项，本环境变量仅作为应急开关）。 |
| `HERMES_STREAM_RETRIES` | 瞬时网络错误时的流中重连尝试次数（默认：`2`，即最多共 3 次尝试）。 |
