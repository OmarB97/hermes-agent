...
The **stale stream detection** kills connections that receive SSE keep-alive pings but no actual content. Local providers get a far more generous ceiling instead of the 180s default — `agent.local_stream_stale_timeout` (900s, or the `HERMES_LOCAL_STREAM_STALE_TIMEOUT` env var), widened further by the request's context-scaled prefill cost so a large prompt on slow hardware is not killed mid-prefill. It stays finite on purpose: a crashed or deadlocked local server has to eventually trip the detector so reconnect and fallback can run, rather than parking the session forever. Before the timer is allowed to kill a local connection, Hermes asks the server whether it is still working — if the backend reports an in-flight generation, the wait is extended rather than thrown away, up to a hard 30-minute bound on any single silent stretch. Only local backends that publish a progress signal can answer that question; a plain Ollama or llama.cpp is decided by the timer exactly as before. Set it to `0` if you want the old unbounded wait back, or set `HERMES_STREAM_STALE_TIMEOUT` / `providers.<id>.stale_timeout_seconds` to pin one value everywhere.

The **stale non-stream detection** kills non-streaming calls that produce no response for too long. By default Hermes disables this on most local endpoints to avoid false positives during long prefills. Two kinds of local model keep a finite budget anyway: the local DeepSeek Flash family, which has its own tighter budget, and models with a known long thinking phase (DeepSeek R1, QwQ, Qwen3, Nemotron 3, the o-series), which get a 300–600s floor. If you explicitly set `providers.<id>.stale_timeout_seconds`, `providers.<id>.models.<model>.stale_timeout_seconds`, or `HERMES_API_CALL_STALE_TIMEOUT`, that explicit value is honored even on local endpoints. Whenever the detector is live on a local endpoint, Hermes asks the server whether it is still working before killing anything — a backend that reports an in-flight generation gets its wait extended rather than thrown away, up to a hard 30-minute bound on the request. This matters more than in the streaming case: a non-streaming call returns nothing until it returns everything, so a kill discards the entire prefill and generation. Only local backends that publish a progress signal can answer; a plain Ollama or llama.cpp is decided by the timer exactly as before.
