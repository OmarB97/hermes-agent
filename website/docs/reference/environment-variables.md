...
| `HERMES_API_CALL_STALE_TIMEOUT` | Non-streaming stale-call timeout in seconds (default: `90`). Auto-disabled for local providers when left unset — except the local DeepSeek Flash family and models with a known long thinking phase (DeepSeek R1, QwQ, Qwen3, Nemotron 3, o-series), which keep a finite budget — and may scale upward for very large contexts. Whenever the detector is live on a local endpoint, a backend that publishes a progress signal is asked whether it is still generating before its request is killed, and the wait is extended if it is, bounded by `HERMES_DFLASH_FIRST_CHUNK_CEILING` (default `1800`). Endpoints without such a signal (a plain Ollama or llama.cpp) are decided by the timer alone. Also configurable via `providers.<id>.stale_timeout_seconds` or `providers.<id>.models.<model>.stale_timeout_seconds` in `config.yaml`. |
...
| `HERMES_STREAM_STALE_TIMEOUT` | Stale stream detection timeout in seconds (default: `180`). Triggers connection kill if no chunks arrive within this window. Local providers use the far more generous `HERMES_LOCAL_STREAM_STALE_TIMEOUT` ceiling instead, unless you set this explicitly — then your value applies everywhere. |
| `HERMES_LOCAL_STREAM_STALE_TIMEOUT` | Stale stream ceiling for local providers (Ollama, oMLX, llama-cpp) in seconds (default: `900`). Applies when a local endpoint is detected and nothing more specific is set — `providers.<id>.stale_timeout_seconds` and `HERMES_STREAM_STALE_TIMEOUT` both win outright, and the local DeepSeek Flash family has its own tighter budget. This finite ceiling replaces the former infinite disable so a wedged local server eventually trips the detector instead of hanging forever. It is widened (never narrowed) by the request's context-scaled prefill cost, so a large prompt on a slow local model is not killed mid-prefill; `HERMES_DFLASH_FIRST_CHUNK_CEILING` (default `1800`) still caps the result. When the ceiling expires, a local backend that publishes a progress signal is asked whether it is still generating, and the wait is extended if it is — bounded by that same `HERMES_DFLASH_FIRST_CHUNK_CEILING`, which is the longest any single silent stretch may run. Endpoints without such a signal (a plain Ollama or llama.cpp) are decided by the timer alone. Set `0` to disable the watchdog entirely. Also configurable via `agent.local_stream_stale_timeout` in `config.yaml` (the canonical setting — this variable is the escape hatch). |
| `HERMES_STREAM_RETRIES` | Number of mid-stream reconnect attempts on transient network errors (default: `2`, i.e. up to 3 attempts in total). |
